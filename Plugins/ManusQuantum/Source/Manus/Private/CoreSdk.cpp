...
//FCriticalSection* CoreSdk::s_CriticalSectionExtraData = new FCriticalSection();
//ErgonomicsStream* CoreSdk::s_ExtraDataData = nullptr;
...
EManusRet CoreSdk::ConnectRemotely(FString p_Host)
{
	...
	if (t_ReturnCode == EManusRet::Success)
	{
		// optional to check for the host first? we already did that in this example, so no official need for it. and it delays it by 2 seconds.
		// but if you want to be extra careful, you can uncomment the following step.
		/*
		// first search for hosts.
		t_ReturnCode = (EManusRet)s_FunctionPointers->LookForHosts(s_SecondsToFindHosts, false);
		if (t_ReturnCode != EManusRet::Success)
...
EManusRet CoreSdk::CheckConnection()
{
	...
	RETURN_IF_NOT_INITIALISED(TEXT("GetIsConnectedToCore"), EManusRet::Error);
